...
The short version: read the open notes, ask Julian what to do with each one using multiple-choice questions, apply the answers to `index.html` and to the `journal_notes` table, push.
...
1. **Fold it in.** Edit the journal so the section reflects the note, then mark the note resolved. Say in one line what edit you would make, so Julian can correct the interpretation before you touch anything.
2. **Keep it visible.** Mark the note `kept`. No edit. This is the right answer for jokes, in-character objections, and useful context that is not a correction. It always has to be on the list.
3. **Resolve without changes.** Mark resolved with a resolution such as "Already covered in Session 1 minutes" or "Duplicate of an earlier note". No edit.
4. **Skip for now.** Leave it open. Ask nothing more about it this round.
...
Julian can do all of this without an agent: read open notes in the Supabase Table Editor (filter `resolved = false`, `kept = false`), edit `index.html`, set `kept` or `resolved` plus `resolution` on each row in the editor, commit, push.
